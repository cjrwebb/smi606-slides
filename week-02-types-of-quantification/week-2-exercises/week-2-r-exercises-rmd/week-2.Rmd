...
By default, the `quantile()` function will return the "0th" percentile (the minimum/smallest value); the 25th percentile (the lower bound of a 'quartile' - split into four parts - range); the 50th percentile (which is equivalent to the median); the 75th percentile (the upper bound of a 'quartile'); and the "100th" percentile (the maximum/largest value). Here is an example of using the quantile function with the `eqvinc` variable.
...
Write the code required to load the `janitor` package from the library in `R`. Remember, you will need to have installed `janitor` first using `install.packages("janitor")` in the console if you haven't already.
